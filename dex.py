...
def process_tokens(score_weights):
    ...
    for token in data['data']:
        quote = token['quote'][0]  # Pegando o primeiro item do array 'quote'

        token_data = {
            'scroll_id': token['scroll_id'],
            'contract_address': token['contract_address'],
            'name': token['name'],
            'base_asset_id': token['base_asset_id'],
            'base_asset_ucid': token['base_asset_ucid'],
            'base_asset_name': token['base_asset_name'],
            'base_asset_symbol': token['base_asset_symbol'],
            'base_asset_contract_address': token['base_asset_contract_address'],
            'quote_asset_id': token['quote_asset_id'],
            'quote_asset_ucid': token['quote_asset_ucid'],
            'quote_asset_name': token['quote_asset_name'],
            'quote_asset_symbol': token['quote_asset_symbol'],
            'quote_asset_contract_address': token['quote_asset_contract_address'],
            'dex_id': token['dex_id'],
            'dex_slug': token['dex_slug'],
            'network_id': token['network_id'],
            'network_slug': token['network_slug'],
            'last_updated': token['last_updated'],
            'created_at': token['created_at'],
            'convert_id': quote['convert_id'],
            'price': quote['price'],
            'price_by_quote_asset': quote['price_by_quote_asset'],
    ...
    melhores_tokens = best_tokens[['scroll_id',
                                    'contract_address',
                                    'name',
                                    'base_asset_id',
                                    'base_asset_ucid',
                                    ...
                                    'liquidity',
                                    'fully_diluted_value']].to_dict(orient='records')
